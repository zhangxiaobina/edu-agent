...
def test_mcp_lists_all_tools(provider):
    """MCP 发现与本地 registry 同源；无健康 Jobe 时 run_code 不可见。"""
    visible_local = {
        item["function"]["name"]
        for item in registry.openai_tools()
    }
    assert set(provider.tool_names()) == visible_local
    assert "run_code" not in provider.tool_names()
...
if __name__ == "__main__":  # 允许脱离 pytest 直接跑
    ...
    try:
        ...
        test_mcp_write_cannot_bypass_transaction_runtime(p)
