...
def test_registry_parity():
    ...
    assert "run_code" not in {tool["function"]["name"] for tool in tools}
...
def test_run_code_paths():
    result = registry.dispatch("run_code", {"source_code": "print(1+1)", "expected_output": "2"})
    assert "error" in result
    assert "provider" in result["error"] or "后端" in result["error"]
