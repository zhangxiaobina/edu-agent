...
def test_new_database_has_minimal_journal_schema_and_strict_snapshot(tmp_path):
    ...
    assert user_version == 9 and migration_count == 1
    assert not {
        "plan_json",
        "evidence_json",
        "operation_json",
...
def test_newer_database_schema_is_never_downgraded(tmp_path):
    ...
    with sqlite3.connect(marker_path) as connection:
        connection.executescript(
            ...
            INSERT INTO state_schema_migrations VALUES ('010_future', 't0');
            """
        )
    with pytest.raises(StateSchemaVersionError, match="010_future"):
