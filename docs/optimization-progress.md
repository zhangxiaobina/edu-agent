...
- container: 新增 `deploy/api/Dockerfile` 多阶段 Python 3.12 镜像，Python 与 UV 使用已核验的多架构 registry digest，依赖通过 `uv sync --frozen --no-dev --no-editable` 从 `uv.lock` 安装；最终镜像 UID/GID `10001`，只复制 `edu_agent`、API/preflight/maintenance 脚本和无凭据 config example。`.dockerignore` 排除 `.git`、`.env*`、key/cert、数据库/WAL/SHM、`dpo_dumps`、评测 artifact、tests/cache。
- startup/health: `scripts/container_preflight.py` 不构造模型或访问 Provider，只执行配置解析、幂等 StateStore migration、SQLite `integrity_check`/`foreign_key_check`、Artifact/状态目录写探测和 rollback-only readiness probe；失败返回稳定类型并退出。`entrypoint.sh` 读取外部 secret file、先 preflight 后 `exec` API。Compose healthcheck 使用 `/health/ready`；现有 R4 lifecycle 保持 `starting/running/draining/stopped`、SIGTERM drain、liveness 保持成功、readiness 在 drain 失败，外部模型瞬时故障不改变进程存活。
- deployment: 新增 `deploy/docker-compose.yml`，仅包含 API；loopback port、named state/artifact mount、explicit backup bind mount、read-only rootfs、64 MiB tmpfs、非 root、`cap_drop=ALL`、`no-new-privileges`、`restart=unless-stopped` 和 40s stop grace。Jobe/Docker socket 不挂入 API。`scripts/container_smoke.py --docker` 在有 daemon 的主机上构建 throw-away compose 项目，覆盖 non-root、私有文件、只读 rootfs、卷重启持久性、restart、SIGTERM audit transition、backup/restore 和 API health smoke。
- verification: `uv lock --check`、`uv pip check --python .venv/bin/python`、目标 ruff、容器静态专项 `7 passed, 1 skipped`、获准回环环境全量 `675 passed, 1 skipped (44.54s)`；公开 `zsh scripts/accept_stage8.sh` development 门禁也通过（包含 `33 passed` Stage 8 边界、`104 passed` R4 关键组、`148 passed` R2 组、最终全量 `675 passed`、lineage/context/Trace/backup/data-boundary/coverage 审计）。`scripts/container_smoke.py` 报告 `static_status=verified`，runtime checks 全部 `not_verified`。当前无 Docker daemon，镜像 build、非 root inspect、只读 rootfs/volume/restart/SIGTERM、容器内 backup/restore/API smoke 均未宣称通过；受限沙箱初跑的 14 个回环 bind 失败不计入回归结论。
...
- minimal fixes: 修正 SHA-256 被手机号规则误脱敏；R5.2 candidate/release 在读取凭据和联网前拒绝脏 Git，并只允许写入忽略的 `ci-artifacts/` 或仓库外；Stage 8 candidate/release 同样将 system/Trace/lineage/audit 写入 `ci-artifacts/`，避免 evidence 自身污染 provenance。R5.4 报告把 R5.2 明确分类为旧 development evidence；README、architecture、eval、demo、interview、roadmap 和项目 description 删除或降级真实平台、实时模型和当前候选的无证据表述。
- verification: full Ruff 0 diagnostics；全量 pytest `689 passed, 1 skipped`；完整 development `zsh scripts/accept_stage8.sh` 退出 0（Stage 8 边界 `34 passed`、R4 `106 passed`、R2 `148 passed`、最终 `689 passed, 1 skipped`）。定向故障矩阵合计 `73 passed`；其中 3 个 socket/SIGTERM 用例只因受限沙箱禁止 loopback bind 首轮失败，在获准本机环境复跑 `3 passed`。唯一 skip 是无 Docker daemon 的容器 E2E。
- evidence: R5.4 正常/故障报告分别 17/17、18/18 assertions；故障路径一份非空 state + 1 个受管 Artifact 的 CLI backup/verify/restore/verify-state 全过，schema 16、1 session/run/journal/operation/checkpoint/artifact、0 foreign-key violations。10k Trace 为 10,000 indexed / 10,001 exported、3/3 assertions；lineage 为 73（55/12/6）、manifest `163e5d23...a68ab43`；25 个受跟踪 artifact 数据审计 0 findings，本地 Markdown 链接无缺失。
