...
`harness.py` 运行器）。环境按仓库根目录 README 使用 uv 准备；运行：
`uv run --frozen python scripts/eval_demo.py [--engine oracle|openai]`。系统级分栏报告使用
`uv run --frozen python scripts/eval_system.py --output artifacts/system-eval.json`；结果自带时间、
版本/commit、环境、模型、seed 与配置 hash，不从本文读取“当前指标”。
...
uv run --frozen python scripts/eval_demo.py --engine openai
```

跑出后保存原始 JSON 与脱敏失败轨迹；README 只保留方法和定性边界，不把一次本地数字写成长期结论。
...
**历史实验：多步早停/幻觉的提示词修复 before→after**（同端点顺序跑，隔离
系统提示词这一单一变量；`scripts/eval_ablation.py` 一次跑两档出对照）的定性结果：
...
- **当前设计**：保留 `edu_agent/agent/prompts.py` 中验证过的多步执行纪律，删除运行时
  `reflect` 节点和合成 user 消息。生产循环只允许真实 user 输入、assistant 工具调用和 tool
  返回，避免破坏角色语义与长会话缓存稳定性。
- **代价与边界（诚实）**：提示词只能缓解早停，**救不了模型层面**的选错工具 / 不敢调写操作 /
  长链自信编造；下一阶段应使用显式计划状态与确定性完成验证器，而不是继续注入反思消息。
  个别多步任务为边界任务，受 vLLM 非完全确定性在边界处轻微波动。
- **复现**：`uv run --frozen python scripts/eval_ablation.py`（before/after 两档）；
  `uv run --frozen python scripts/eval_demo.py --engine openai`（单档完整 19 任务）；
  `uv run --frozen python scripts/eval_subset.py --cats multi_step`（快速多步子集评测）。

## 6. PlanGraph before/after 口径

`scripts/eval_plan_ablation.py` 保持同一模型、温度、system prompt、seed-42 数据库和 19-task 顺序，
唯一变量是 `before=关闭 PlanGraph`、`after=只对 multi_step 开启 PlanGraph`。除了原指标，还报告：

- multi_step 步骤完成率与提前结束率；
- 工具选择 precision/recall/F1；
- 平均模型调用数和工具调用数，planner 调用包含在模型预算内；
- irrelevance 分组，确认计划层没有把无关问题转成工具调用。

离线命令 `uv run --frozen python scripts/eval_plan_ablation.py --engine oracle` 实测 before/after 均
为 19/19、multi_step 完成率 100%、提前结束率 0%；计划档平均模型调用 3.67→4.67，平均工具调用
2.67→2.67。oracle 按期望轨迹和严格计划回放，因此这些数字**只证明 harness、证据绑定和成本统计
可运行**，不代表真实模型提升。

真实数据命令为 `uv run --frozen python scripts/eval_plan_ablation.py --engine openai`。PlanGraph 实现验收时
没有确认可用的真实模型端点，故**真模型数据未运行**；历史提示词实验也不能替代本次 PlanGraph
严格消融。
