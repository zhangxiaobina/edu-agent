...
def score_task(task: EvalTask, result: dict) -> dict:
    rec = {
        ...
        "model_calls": result.get("budget", {}).get("model_calls", 0),
        "tool_calls": result.get("budget", {}).get(
            "tool_calls",
    ...
    if task.expected_tools:                   # single/multi_step/parallel/relevance
        ...
        if plan and plan.get("steps"):
            ...
            rec["step_completion_rate"] = completed / len(plan["steps"])
        else:
            rec["step_completion_rate"] = rec["tool_recall"]
        rec["early_termination"] = bool(
            task.category == "multi_step"
            and result.get("final_answer")
            and rec["tool_recall"] < 1.0
        )
...
def aggregate(records: list[dict]) -> dict:
    ...
    for cat in sorted({r["category"] for r in records}):
        ...
        by_category[cat] = {
            ...
            "tool_recall": _mean([r["tool_recall"] for r in crs if "tool_recall" in r]),
            "tool_precision": _mean(
                [r["tool_precision"] for r in crs if "tool_precision" in r]
            ),
            "param_accuracy": _mean([r["param_accuracy"] for r in crs if "param_accuracy" in r]),
            "step_completion_rate": _mean(
                [r["step_completion_rate"] for r in crs if "step_completion_rate" in r]
            ),
            "early_termination_rate": _mean(
                [r["early_termination"] for r in crs if "early_termination" in r]
            ),
            "avg_model_calls": _mean([r["model_calls"] for r in crs]) or 0.0,
            "avg_tool_calls": _mean([r["tool_calls"] for r in crs]) or 0.0,
        }
    return {
        "n_tasks": n,
        ...
        "step_completion_rate": _mean(
            [r["step_completion_rate"] for r in tool_recs]
        ),
        "early_termination_rate": _mean(
            [r["early_termination"] for r in records if "early_termination" in r]
        ),
        "avg_model_calls": _mean([r["model_calls"] for r in records]) or 0.0,
        "avg_tool_calls": _mean([r["tool_calls"] for r in records]) or 0.0,
