...
def run_eval(tasks: list[EvalTask], make_engine: Callable[[EvalTask], Engine],
             ...
             run_options: Callable[[EvalTask, Engine], dict] | None = None) -> dict:
    ...
    for task in tasks:
        ...
        try:
            options = run_options(task, engine) if run_options else {}
...
def format_report(report: dict) -> str:
    lines = [
        ...
        f"步骤完成率                     : {_pct(report['step_completion_rate'])}",
        f"提前结束率                     : {_pct(report['early_termination_rate'])}",
        f"平均模型/工具调用              : {report['avg_model_calls']:.2f} / "
